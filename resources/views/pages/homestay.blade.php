...
<x-app-layout>
    <main id="main-content" class="page-hero" style="background-image: url('{{ asset('assets/homestay/homestaybg.png') }}');">
                ...
                <div class="grid grid-cols-4 card-padded-lg" style="background: white; border-radius: var(--radius-2xl); overflow: hidden; box-shadow: var(--shadow-sm); border: 1px solid var(--color-gray-200); margin-bottom: 3rem;">
                    @foreach ($homestays as $home)
                        <article class="card" style="display: flex; flex-direction: column; overflow: hidden; background: white; border: 1px solid var(--color-gray-200);">
                            <div class="card-image-wrap card-img-wrap">
                                @if(Str::startsWith($home->main_photo, 'http'))
                                    <img src="{{ $home->main_photo }}" alt="{{ $home->name }}" class="card-image card-img" />
                                @else
                                    <img src="{{ asset(Str::startsWith($home->main_photo, 'storage/') ? $home->main_photo : 'storage/'.$home->main_photo) }}" alt="{{ $home->name }}" class="card-image card-img" onerror="this.src='{{ asset($home->main_photo) }}'" />
                                @endif
                            </div>
