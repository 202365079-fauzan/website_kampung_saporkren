...
<x-app-layout>
    <main id="main-content" class="page-hero" style="background-image: url('{{ asset('assets/bird/birdwatchingbg.png') }}');">
            ...
            <section class="py-8">
                <div class="card card-padded-lg" style="margin-bottom: 3rem;">
                    <div class="section-header">
                        ...
                        <h2 class="section-title section-title-sm">3 Burung Endemik Unggulan Kampung Saporkren</h2>
                        <p class="section-desc">Tiga spesies burung surga paling dicari peneliti dan pecinta burung dari seluruh dunia.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-3" style="gap: 2rem;">
                        @foreach ($birdSpeciesList as $spec)
                            ...
                            @endphp
                            <article class="product-article" style="height: 100%;">
                                <div class="card-img-wrap">
                                    @if($image)
                                        @if(Str::startsWith($image, 'http'))
                                            <img src="{{ $image }}" alt="{{ $localName }}" class="card-img" />
                                        @else
                                            <img src="{{ asset(Str::startsWith($image, 'storage/') ? $image : 'storage/'.$image) }}" alt="{{ $localName }}" class="card-img" onerror="this.src='{{ asset($image) }}'" />
                                        @endif
                                    @endif
                                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; margin-top: auto; font-size: 0.8rem;">
                                        <div class="info-cell">
                                            <span class="info-cell-label">Habitat:</span>
                                            <span class="info-cell-value">{{ $habitat }}</span>
                                        </div>
                                        <div class="info-cell">
                                            <span class="info-cell-label">Waktu Pengamatan:</span>
                                            <span class="info-cell-value">{{ $bestTime }}</span>
                                        </div>
                                    </div>
                                </div>
                            </article>
            ...
            <section class="py-8">
                <div class="card card-padded-lg" style="margin-bottom: 3rem;">
                    <div class="section-header">
                        ...
                        <h2 class="section-title section-title-sm">Paket Bird Watching</h2>
                        <p class="section-desc">Pilihan pengalaman mengamati keindahan burung Cenderawasih bersama pemandu lokal.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-2" style="gap: 2rem;">
                        @foreach ($birdPackages as $pack)
                            <article class="card-padded" style="background: var(--color-gray-50); border: 1px solid var(--color-gray-200); border-radius: 1.5rem; display: flex; flex-direction: column;">
                                <span class="category-label">{{ $pack->duration }}</span>
                                ...
                                <ul style="list-style: none; margin-top: 1.5rem; display: flex; flex-direction: column; gap: 0.75rem; font-size: 0.9rem; color: var(--color-gray-600);">
                                    @if(is_array($pack->includes))
                                        @foreach ($pack->includes as $inc)
                                            <li class="list-bullet-start">
                                                <span class="bullet-dot-md" style="margin-top: 0.4rem;"></span>
                                                <span>{{ $inc }}</span>
                                            </li>
                                        @endforeach
                                    @endif
